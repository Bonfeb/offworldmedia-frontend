...
const AdminReviews = () => {
  ...
  const fetchReviews = async (filters = {}) => {
    ...
    try {
      ...
      const response = await API.get(`/admin-dashboard/?${queryParams.toString()}`);
  ...
  const hasActiveFilters = Object.values(searchFilters).some(value => value && value.toString().trim());
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) => handleFilterChange('rating__gte', e.target.value)}
              >
                <MenuItem value="">
                  <em>No Minimum</em>
                </MenuItem>
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) => handleFilterChange('rating__lte', e.target.value)}
              >
                <MenuItem value="">
                  <em>No Maximum</em>
                </MenuItem>
      ...
      <Card className="review-card my-3">
        <Card.Body>
          ...
          {(!error && reviews.length === 0) && (
            <Alert severity="info">
              {hasActiveFilters ? 'No reviews match your search criteria.' : 'No reviews found.'}
            </Alert>
          )}
          {reviews.map((review) => (
