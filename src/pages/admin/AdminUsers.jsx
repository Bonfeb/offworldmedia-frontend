...
import { Container, Table, Button, Modal, Form, Spinner, Card, Row, Col } from 'react-bootstrap';
import { 
  Edit as EditIcon, 
...
const AdminUsers = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await API.put(`/admin-dashboard/?action=user&user_id=${currentUser.id}`, formData);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await API.delete(`/admin-dashboard/?action=user&user_id=${currentUser.id}`);
      fetchUsers(); // Refresh the user list
  ...
  const toggleRowExpansion = async (userId) => {
    ...
    if (!newExpandedRows[userId] && !userDetails[userId]) {
      try {
        ...
        const details = await API.get(`/admin-dashboard/?action=user-details&user_id=${userId}`);
        setUserDetails(prev => ({
          ...prev,
          [userId]: details.data
        }));
